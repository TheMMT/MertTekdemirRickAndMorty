...
export const TagsInput = React.forwardRef<HTMLDivElement, TagsInputProps>(
  (
    ...
    const [inputWidth, setInputWidth] = React.useState("100px"); // Başlangıç genişliği
    const spanRef = React.useRef<HTMLSpanElement>(null); // Span için ref
    ...
    // Input genişliğini dinamik olarak ayarlama
    React.useEffect(() => {
      if (spanRef.current) {
    ...
    const handleKeyDown = React.useCallback(
      ...
      [activeIndex, value, inputValue, RemoveValue, dir],
    );
    ...
    return (
      ...
      >
        <div
          ...
          dir={dir}
          className={cn(
            "relative flex items-center flex-wrap gap-1 p-1 rounded-lg bg-background overflow-hidden ring-1 ring-muted",
            {
        ...
        >
          ...
          <Input
            ...
            className={cn(
              "outline-0 border-none h-7 min-w-[100px] focus-visible:outline-0 focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-0 placeholder:text-muted-foreground px-1",
              activeIndex !== -1 && "caret-transparent",
            )}
            style={{ width: inputWidth }} // Dinamik genişlik
          />
          <span
            ref={spanRef}
          ...
          >
            {inputValue || placeholder} {/* Input boşsa placeholder gösterilir */}
          </span>
        </div>
      </TagInputContext.Provider>
    );
